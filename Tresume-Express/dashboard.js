...
router.post('/getAdminDashboardData', async (req, res) => {
    ...
    try {
        const [
            ...
            pool.query(`SELECT CONCAT(T.FirstName, ' ', T.LastName) AS Recruiter, (SELECT COUNT(id) FROM division_audit WHERE username = T.Username AND jobboardid = 3 AND createtime BETWEEN '${startdate}' AND '${enddate}') AS monsterused, (SELECT COUNT(id) FROM division_audit WHERE username = T.Username AND jobboardid = 4 AND createtime BETWEEN '${startdate}' AND '${enddate}') AS cbused, (SELECT COUNT(id) FROM division_audit WHERE username = T.Username AND jobboardid = 2 AND createtime BETWEEN '${startdate}' AND '${enddate}') AS diceused FROM Trainee AS T INNER JOIN org_division AS OD ON T.Org_Div = OD.id WHERE T.organizationid IN (${accessorg}) AND T.Active = 1 ORDER BY OD.DivisionName;`),
